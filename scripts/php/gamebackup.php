...
if ($gameresult = mysqli_query($con, "SELECT * FROM Game ORDER BY 'gameID' ASC")) {
    ...
    while ($row = mysqli_fetch_assoc($gameresult)) {
        ...
        GameID = '" . $row['GameID'] . "', 
        homeTeamName = '" . mysqli_real_escape_string($con, $row['homeTeamName']) . "', 
        homeTeamScore = '" . $row['homeTeamScore'] . "', 
        awayTeamName = '" . mysqli_real_escape_string($con, $row['awayTeamName']) . "', 
        awayTeamScore = '" . $row['awayTeamScore'] . "', 
        minutesPlayed = '" . $row['minutesPlayed'] . "', 
        ref = '" . mysqli_real_escape_string($con, $row['ref']) . "', 
        assRef1 = '" . mysqli_real_escape_string($con, $row['assRef1']) . "', 
        assRef2 = '" . mysqli_real_escape_string($con, $row['assRef2']) . "', 
        location = '" . mysqli_real_escape_string($con, $row['location']) . "', 
        time = '" . $row['time'] . "', 
        scoringPlays = '" . mysqli_real_escape_string($con, $row['scoringPlays']) . "', 
        changed = '" . $row['changed'] . "', 
        changes = '" . $row['changes'] . "', 
        liveScored = '" . $row['liveScored'] . "', 
        userID = '" . $row['userID'] . "', 
        lastTimeScored = '" . $row['lastTimeScored'] . "', 
        locked = '" . $row['locked'] . "', 
