...
$(document).ready(function () {
    ...
    var post = $.post('http://ccrscoring.co.nz/scripts/php/getallinfo.php', {}, function (response) {
...
function alertError(selector, message) {
    ...
    setTimeout(function () {
        $(selector).next().remove();$('.footer').hide();
    }, 5000);
}
                ...
                var post = $.post('http://ccrscoring.co.nz/scripts/php/changedate.php', {
                    ...
                    if (response == 'success') {
                        ...
                        if (row.hasClass('selectedRow')) {
                            row.removeClass('selectedRow').addClass('selectedRow');
                        }
    ...
    $('tr > [contenteditable=true]').on({
        keydown: function keydown(event) {
            if (event.keyCode == 38 && event.shiftKey) {
                // Shift + arrow up moves up one cell
                ...
                // Shift + arrow down moves down one cell
                ...
                // Shift + arrow left moves left one cell
                // Get the parent of element currently focused, move to next sibling, get the child with the same class name and focus it.
                ...
                // Shift + arrow right moves right one cell
                // Get the parent of element currently focused, move to next sibling, get the child with the same class name and focus it.
...
function createResultsPDF() {
    ...
    for (var z = 0; z < allDivs.length; z += 1) {
        ...
        for (var v = 0; v < gameArray[z].length; v += 1) {
            ...
            body.push([{ text: gameDateString }, // Date
            ...
            { text: 'vs' }, { text: getTeamName(gameID.substr(11, 3), gameID.slice(-2)) }, // Away Team Name
            { text: game.awayTeamScore == 2 ? 'Win' : game.awayTeamScore == 1 ? 'Defaulted' : game.awayTeamScore }]);
        }
...
function createDrawPDF() {
    ...
    for (var z = 0; z < allDivs.length; z += 1) {
        ...
        for (var v = 0; v < gameArray[z].length; v += 1) {
            ...
            body.push([{ text: gameDateString, color: isChanged('date', gameChanges) ? 'red' : 'black' }, // Date
            { text: getTeamName(gameID.substr(8, 3), gameID.slice(-2)) }, // Home Team Name
            { text: 'vs' }, { text: getTeamName(gameID.substr(11, 3), gameID.slice(-2)) }, // Away Team Name
...
function updatePointsTable() {
    var post = $.post('http://ccrscoring.co.nz/scripts/php/updatepointstable.php', {}, function (response) {
